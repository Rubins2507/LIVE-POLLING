...
func (r *RedisClient) ClaimVoter(ctx context.Context, pollID, voterID string) (bool, error) {
	key := fmt.Sprintf("poll:%s:voter:%s", pollID, voterID)
	return r.Client.SetNX(ctx, key, 1, 0).Result()
}

func (r *RedisClient) ReleaseVoter(ctx context.Context, pollID, voterID string) error {
	key := fmt.Sprintf("poll:%s:voter:%s", pollID, voterID)
